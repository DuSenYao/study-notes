...
- [Node.js](#nodejs)
  - [一. Node.js 简介](#一-nodejs-简介)
    ...
    - [1.7 Node.js 安装](#17-nodejs-安装)
      - [1.7.1 npm](#171-npm)
      - [1.7.2 pnpm](#172-pnpm)
  - [二. Node 编程](#二-node-编程)
    - [2.1 Node 基础](#21-node-基础)
      - [2.1.1 控制台输出](#211-控制台输出)
      - [2.1.2 命令行参数和环境变量](#212-命令行参数和环境变量)
      - [2.1.3 程序生命期](#213-程序生命期)
      - [2.1.4 Node 模块](#214-node-模块)
    - [2.2 Node 默认异步](#22-node-默认异步)
      - [2.2.1 并发](#221-并发)
    - [2.3 缓冲区](#23-缓冲区)
    - [2.4 事件与 EventEmitter](#24-事件与-eventemitter)
    - [2.5 流](#25-流)
      - [2.5.1 管道](#251-管道)
    - [2.5.2 异步迭代](#252-异步迭代)
      - [2.5.3 写入流及背压处理](#253-写入流及背压处理)
      - [2.5.4 通过事件读取流](#254-通过事件读取流)
    - [2.6 进程、CPU 和操作系统细节](#26-进程-cpu-和操作系统细节)
    - [2.7 操作文件](#27-操作文件)
      - [2.7.1 路径、文件描述符和 FileHandle](#271-路径-文件描述符和-filehandle)
      - [2.7.2 读文件](#272-读文件)
      - [2.7.3 写文件](#273-写文件)
      - [2.7.4 文件操作](#274-文件操作)
      - [2.7.5 文件元数据](#275-文件元数据)
    - [2.7.6 操作目录](#276-操作目录)
    - [2.8 HTTP 客户端与服务器](#28-http-客户端与服务器)
    - [2.9 非 HTTP 网络服务器及客户端](#29-非-http-网络服务器及客户端)
    - [2.10 操作子进程](#210-操作子进程)
      - [2.10.1 execSync() 与 execFileSync()](#2101-execsync-与-execfilesync)
      - [2.10.2 exec() 与 execFile()](#2102-exec-与-execfile)
      - [2.10.3 spawn()](#2103-spawn)
      - [2.10.4 fork()](#2104-fork)
    - [2.11 工作线程](#211-工作线程)
      - [2.11.1 创建工作线程及传递消息](#2111-创建工作线程及传递消息)
      - [2.11.2 工作线程的执行环境](#2112-工作线程的执行环境)
      - [2.11.3 通信信道与 MessagePort](#2113-通信信道与-messageport)
      - [2.11.4 转移 MessagePort 和定型数组](#2114-转移-messageport-和定型数组)
      - [2.11.5 在线程间共享定型数组](#2115-在线程间共享定型数组)
...
Node 也遵循了这些 Unix 惯例。Node 程序可以从字符串数组 `process.argv` 中读取其命令行参数。这个数组的第一个元素始终是 Node 可执行文件的路径。第二个参数是 Node 执行的 JS 代码文件的路径。数组中剩下的所有元素都是在调用 Node 时，通过命令行传给它的空格分隔的参数。
...
- **hex**：这种编码把每个字节转换为一对 ASCII 十六进制数字。
...
**os 模块（与 process 不同，需要通过模块语法引入）提供对 Node 所在计算机和操作系统的类似的低级细节**。尽管可能永远用不到这些特性，但应该知道 Node 暴露了这些信息：
...
这里介绍的各种文件写入函数在数据已经 “写入” 时（对 Node 而言就是把数据交给了操作系统），会返回或调用它们的回调或解决它们的期约。但这不一定意味着数据已经事实上写入了持久存储系统。至少其中有些数据仍然缓存在操作系统或设备驱动的某个地方，等待写入磁盘。如果调用了 `fs.writeSync()` 同步向文件中写入某些数据，刚好在这个函数返回时掉电了，那可能会丢失数据。如果想把数据强制写入磁盘，保证数据得到安全存储，可以使用 `fs.fsync()` 或 `fs.fsyncSync()`。这两个函数只接收文件描述符，没有接收路径的版本。
...
1. 创建一个新 Server 对象。
2. 调用它的 listen() 方法，开始监听指定端口的请求。
3. 为 “request” 事件注册处理程序，通过这个处理程序读取客户端请求（特别是 `url` 属性），然后写入响应。

下面的代码将创建一个简单的 HTTP 服务器，可以发送本地文件系统的静态文件，同时也实现了一个调试终端，可以将客户端请求再发送给客户端：

```js
// 这是一个简单的静态 HTTP 服务器，从一个指定目录发送文件
...
除了编写高并发服务器，Node 也非常适合编写执行其他程序的脚本。Node 中的 chidprocess 模块定义了一些函数，用于在子进程中运行其他程序。

#### 2.10.1 execSync() 与 execFileSync()
...
启动子进程的代价是相当大的，如果子进程不能完成几个大数量级的计算，那么就不值得像这样使用 `fork()` 和进行进程间通信。如果程序需要保证对到来的事件快速响应，也需要执行耗时计算，那可以考虑使用一个独立的子进程去执行计算，从而不阻塞事件循环，也不影响父进程的响应速度（不过，在这种情况下，可能使用[线程](#211-工作线程)比使用进程更好）。
