...
- [函数式编程](#函数式编程)
  ...
  - [四. 组合和管道](#四-组合和管道)
    ...
    - [4.2 函数式组合](#42-函数式组合)
      ...
      - [7.3.1 引入 curry 与 partial](#731-引入-curry-与-partial)
...
上面的文字清晰地给出了抽象的理念。**抽象让我们专注于预定的目标，而不必关心底层的系统概念**。
...
let curry = fn => {
  ...
  return function curriedFn(...args) {
    if (args.length < fn.length) {
      ...
      return (...args2) => curriedFn.apply(null, [...args, ...args2]); // 将一次传入一个参数放入数组中并递归调用 curriedFn
    }
...
归根结底，我们使用 curry 或 partial 是为了让函数参数或函数设置变得更加简单和强大！
...
compose 函数需要接收一个函数的输出，并将其用作输入以传递给另一个函数。
...
输出是我们期望的 4。可见，data(parseFloat 函数的输出) 作为输入被传递给 Math.round 以获得结果，这是 compose 函数能够解决的典型问题。下面通过 compose 函数解决该问题：
