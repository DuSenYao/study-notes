...
除了这三个语句和一个特例 `for (<let/const>...) ...` 之外，所有其它的语句都是没有块级作用域的。例如 if 条件语句的几种常见书写形式：
...
并不是所有的循环语句都有自己的块级作用域，例如 while 和 do/while 语句就没有。而且，也不是所有 for 语句都有块级作用域。在 JS 中，有且仅有：

> `for (<let/const> ...) ...`

这个语法有自己的块级作用域。当然，这也包括相同设计的 for await...of 和 for...of/in...。例如：
