...
- [JS 代码规范](#js-代码规范)
  - [一. 命名规范](#一-命名规范)
    ...
    - [1.2 js 中普通变量使用 小写开头驼峰命名法，而非不区分大小写，或使用下划线命名等等](#12-js-中普通变量使用-小写开头驼峰命名法而非不区分大小写或使用下划线命名等等)
  ...
  - [二. ES6](#二-es6)
    ...
    - [2.2 在 `class` 或 `Object` 中使用新的函数声明方式](#22-在-class-或-object-中使用新的函数声明方式)
    - [2.3 优先使用 箭头函数 而不是使用传统的函数，尤其是使用 匿名函数 时](#23-优先使用-箭头函数-而不是使用传统的函数尤其是使用-匿名函数-时)
    - [2.4 不要使用 `if` 判断后再赋予默认值](#24-不要使用-if-判断后再赋予默认值)
    - [2.5 优先使用 Map 做键值对映射而非传统的对象](#25-优先使用-map-做键值对映射而非传统的对象)
    ...
    - [2.11 简单的选项列表优先使用 `Map` 而非数组](#211-简单的选项列表优先使用-map-而非数组)
    - [2.12 存放 id 标识列表使用 Set 而非数组](#212-存放-id-标识列表使用-set-而非数组)
  - [三. 逻辑代码](#三-逻辑代码)
...
### 1.2 js 中普通变量使用 小写开头驼峰命名法，而非不区分大小写，或使用下划线命名等等
...
### 2.2 在 `class` 或 `Object` 中使用新的函数声明方式
...
### 2.3 优先使用 箭头函数 而不是使用传统的函数，尤其是使用 匿名函数 时
