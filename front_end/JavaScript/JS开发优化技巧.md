...
- [JS 开发优化技巧](#js-开发优化技巧)
  ...
  - [二. 函数类](#二-函数类)
    - [2.1 用异或运算交换两个整数的值](#21-用异或运算交换两个整数的值)
...
当创建了新变量，有时候想要检查引用的变量是不是为非 null 或 undefined。JavaScript 有一个很好的快捷方式来实现这种检查。
...
通过使用箭头函数，可以直接返回值，不需要 `return` 语句。
...
为了重复操作相同的字符，可以使用 `for` 循环，但其实还有一种简便的方法。
