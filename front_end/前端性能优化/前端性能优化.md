...
**应用场景**
从应用的角度来说，一般当脚本与 DOM 元素和其它脚本之间的依赖关系不强时，选用 async
当脚本依赖于 DOM 元素和其它脚本的执行结果时，会选用 defer。
...
- SEO 问题
  搜索引擎只会查找现成的内容，不会运行 JS 代码
- 首屏加载速度过慢

**服务端渲染的应用实例**
**React 实例**
使用 Express 搭建后端服务，项目中有一个叫做 VDom 的 React 组件，它的内容如下。
...
这两个操作是服务端渲染的灵魂操作。在虚拟 DOM"横行"的当下，它还要求这一端要具备将虚拟 DOM 转化为真实 DOM 的能力。
...
**浏览器差异性** 是因为浏览器内核的不同而导致的——浏览器内核决定了浏览器解释网页语法的方式。
浏览器内核可以分成两部分
...
DOM 为什么这么慢，是因为收了 "过路费"。

> 把 DOM 和 JS 各自想象成一个岛屿，它们之间用收费桥梁连接。——《高性能 JavaScript》
...
- 避免逐条改变样式，使用类名去合并样式
- 将 DOM “离线”
  一旦给元素设置 `display: none`，将其从页面上“拿掉”，那么后续操作，将无法触发回流与重绘——这个将元素“拿掉”的操作，就叫做 **DOM 离线化**。
  > 只适用于操作频繁的地方
...
事件循环中的异步队列有两种: macro-task（宏观任务）队列和 micro-task（微观任务）队列。

宿主发起的任务称为宏观任务(macro-task) : 常见的如，setTimeout、setInterval、 setImmediate、script（整体代码）、 I/O 操作、UI 渲染等。
JS 引擎发起的任务称为微观任务(micro-task) : 常见的如，process.nextTick、Promise、MutationObserver 等
...
**渲染的时机**
在异步任务里进行 DOM 更新，应该把它包装成 micro。

用 Promise 来把 task 包装成是一个 micro 任务：
...
**可视化监测 LightHouse**
Lighthouse 是一个开源的自动化工具，用于改进网络应用的质量。可以将其作为一个 Chrome 扩展程序运行，或从命令行运行。 为 Lighthouse 提供一个需要审查的网址，它将针对此页面运行一连串的测试，然后生成一个有关页面性能的报告。Lighthouse 已经集成在 Chrome Tools 里。
